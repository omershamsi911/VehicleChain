    ...
    >
      ...
      <div>
        <div style={{
          ...
          fontSize: 13,
          fontWeight: 600,
          color: isGreen ? "#fff" : isDanger ? "var(--danger)" : "var(--text)",
          marginBottom: 2,
        }}>{label}</div>
        <div style={{
          fontFamily: "var(--font-sans)",
          fontSize: 11,
          color: isGreen ? "rgba(255,255,255,0.75)" : isDanger ? "var(--danger)" : "var(--text-muted)",
          lineHeight: 1.4,
        }}>{desc}</div>
      </div>
    </button>
...
export default function Dashboard({ navigate }) {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      const details = await Promise.all(vins.map(vin => contracts.vehicleRegistry.getVehicle(vin)));
      setVehicles(details);
    } catch (err) {
  ...
  if (!isConnected) {
    return (
      <div style={{
        ...
        justifyContent: "center", minHeight: "70vh", padding: "60px 24px",
        textAlign: "center",
      }}>
        <div style={{
          width: 72, height: 72,
        ...
        <span style={{
          ...
          Decentralised Vehicle Registry
        </span>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", marginBottom: 48 }}>
          {["🔗 Blockchain Verified", "🛡️ Fraud Prevention", "🗳️ DAO Governance"].map(f => (
            <span key={f} style={{
              padding: "8px 16px",
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.pageHeader}>
        ...
        <p style={{ ...S.pageSub, fontFamily: "var(--font-mono)", fontSize: 12, letterSpacing: "0.04em" }}>
      ...
      <div style={S.card}>
        ...
        <form onSubmit={handleSearch} style={{ display: "flex", gap: 10 }}>
          <input
            style={{ ...S.inputFlex, fontFamily: "var(--font-mono)", fontSize: 13, letterSpacing: "0.04em" }}
